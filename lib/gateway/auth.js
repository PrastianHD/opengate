...
export async function authenticate(request) {
  ...
  const { data: key, error } = await sb
    .from("gateway_keys")
    .select(
      "id, user_id, label, enabled, model_whitelist, rpm_cap, spending_cap_micro_cents, spending_used_micro_cents, expires_at, revoked_at"
    )
    .eq("key_hash", keyHash)
    .maybeSingle();
  ...
  const { data: user, error: userErr } = await sb
    .from("users")
    .select("id, email, role, balance_micro_cents, rpm_cap, banned_at")
    .eq("id", key.user_id)
    .single();

  if (userErr || !user) {
    return errResp(500, "user_lookup_failed", userErr?.message || "User missing");
  }
  if (user.banned_at) {
