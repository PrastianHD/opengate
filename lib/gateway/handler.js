...
const MODEL_CACHE_TTL = 60_000;  // 60s — models rarely change
const KEY_CACHE_TTL = 30_000;    // 30s — upstream keys change on cooldown
...
export async function handleChatCompletions(request) {
  ...
  // ---------------- 4. Resolve model (cached) ----------------
  const cacheKey = `model:${requestedModel}`;
  let model = cacheGet(cacheKey);
  if (!model) {
    const sb = createServiceClient();
    const { data, error: modelErr } = await sb
      .from("models")
      .select(
  ...
  cacheSet(cacheKey, model, MODEL_CACHE_TTL);
  ...
  if (wantsStream) {
    ...
    const proxy = makeStreamProxy({
      upstream: upstreamResp,
      adapter,
      slug: model.slug,
    });
  ...
  // Rewrite the upstream model id back to the OpenGate slug so clients only
  // ever see our public catalog, never 9Router internals (e.g. "kr/…").
  ...
  // Fire-and-forget billing — don't block the response.
  settleBilling({
    user,
    key,
    model,
    ...
    isStream: false,
    ttftMs: null,
  }).catch((e) => console.error("billing failed:", e));
